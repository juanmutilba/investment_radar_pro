...
import {
  ...
  getCryptoTestnetStrategyAnalysis,
  ...
  type CryptoTestnetStrategyAnalysisPayload,
...
const TESTNET_ORDER_SUCCESS_MESSAGE = "Orden ejecutada. Posiciones actualizadas.";
...
const TESTNET_DUPLICATE_POSITION_NOTE =
  " En Testnet evitamos duplicar compras del mismo activo. Antes de operar real se revisará esta regla para permitir o no acumulación controlada.";
...
const MONITOR_BE_ACTIVATE_TOOLTIP =
  "Cuando la posición gana este porcentaje, se habilita protección break-even.";
const MONITOR_BE_STOP_TOOLTIP =
  "Nivel de salida cercano al precio de compra para evitar volver a pérdida.";

const MONITOR_FUTURE_AUTO_NOTE =
  "El modo automático con órdenes MARKET está en la pestaña Auto Testnet (sandbox; kill switch y límites).";

/** Etiquetas cortas para auditoría de clamps (parámetros auto testnet). */
const AUTO_PARAM_FIELD_LABELS: Record<string, string> = {
  max_open_positions: "Máx. activos abiertos",
  max_trades_per_day: "Máx. entradas / día",
  quote_amount_usdt: "USDT por entrada",
  max_total_exposure_usdt: "Máx. exposición USDT",
  max_quote_per_order_usdt: "Tope orden USDT",
  cycle_interval_minutes: "Intervalo ciclo (min)",
  min_entry_score: "Score mínimo",
  ...
  max_daily_loss_usdt: "Máx. pérdida día USDT",
  strategy_mode: "Modo estrategia",
  timeframe: "Timeframe",
  require_btc_trend_up: "Requerir BTC alcista",
};
...
const PORTFOLIO_NO_AUTO_EXPAND_NOTE =
  "Tras operar no cambiamos de módulo solo; refrescamos posiciones. Podés seguir en Monitor u Operar.";
...
function autoOpenRiskStatusLabel(code: string | null | undefined): string {
  const c = (code ?? "").trim();
  const labels: Record<string, string> = {
    salida_sugerida: "Salida sugerida",
    cerca_sl: "Cerca SL",
    cerca_tp: "Cerca TP",
...
function CryptoTestnetStrategyModeField({
  ...
  return (
    <label className={className ? `crypto-testnet-field ${className}` : "crypto-testnet-field"}>
      <span className="crypto-testnet-field-label">
...
export function CryptoTestnetPanel() {
  ...
  const activeTestnetModuleRef = useRef<TestnetModule>("status");
  useEffect(() => {
    activeTestnetModuleRef.current = activeTestnetModule;
  }, [activeTestnetModule]);
  ...
  const [autoStatus, setAutoStatus] = useState<CryptoTestnetAutoStatusPayload | null>(null);
  const [autoStatusLoading, setAutoStatusLoading] = useState(false);
  const [autoActionBusy, setAutoActionBusy] = useState(false);
  const [autoCycles, setAutoCycles] = useState<CryptoTestnetAutoCycleRow[]>([]);
  const [autoCyclesLoading, setAutoCyclesLoading] = useState(false);
  const [autoCyclesError, setAutoCyclesError] = useState<string | null>(null);
  ...
  const applyAutoParamsSnapshot = useCallback((p: Record<string, unknown>) => {
    if (typeof p.timeframe === "string" && p.timeframe.trim()) setAutoTf(normalizeTimeframeString(p.timeframe));
    if (p.quote_amount_usdt != null && Number.isFinite(Number(p.quote_amount_usdt))) {
      setAutoQuote(String(p.quote_amount_usdt));
    }
    if (p.max_open_positions != null && Number.isFinite(Number(p.max_open_positions))) {
      setAutoMaxOpen(String(p.max_open_positions));
    }
    if (p.cooldown_minutes != null && Number.isFinite(Number(p.cooldown_minutes))) {
      setAutoCooldown(String(p.cooldown_minutes));
    }
    if (typeof p.require_btc_trend_up === "boolean") setAutoBtcTrend(p.require_btc_trend_up);
    if (p.min_entry_score != null && Number.isFinite(Number(p.min_entry_score))) {
      setAutoMinScore(String(p.min_entry_score));
    }
    if (p.stop_loss_pct != null && Number.isFinite(Number(p.stop_loss_pct))) setAutoSl(String(p.stop_loss_pct));
    if (p.take_profit_pct != null && Number.isFinite(Number(p.take_profit_pct))) setAutoTp(String(p.take_profit_pct));
    ...
    if (p.break_even_plus_pct != null && Number.isFinite(Number(p.break_even_plus_pct))) {
      setAutoBePlus(String(p.break_even_plus_pct));
    }
    if (p.min_exit_value_usdt != null && Number.isFinite(Number(p.min_exit_value_usdt))) {
      setAutoExitMin(String(p.min_exit_value_usdt));
    }
    if (p.cycle_interval_minutes != null && Number.isFinite(Number(p.cycle_interval_minutes))) {
      setAutoCycleMinutes(String(p.cycle_interval_minutes));
    }
    if (p.max_trades_per_day != null && Number.isFinite(Number(p.max_trades_per_day))) {
      setAutoMaxTrades(String(p.max_trades_per_day));
    }
    if (p.max_daily_loss_usdt != null && Number.isFinite(Number(p.max_daily_loss_usdt))) {
      setAutoMaxLoss(String(p.max_daily_loss_usdt));
    }
    if (p.max_total_exposure_usdt != null && Number.isFinite(Number(p.max_total_exposure_usdt))) {
      setAutoMaxExposure(String(p.max_total_exposure_usdt));
    }
    if (p.max_quote_per_order_usdt != null && Number.isFinite(Number(p.max_quote_per_order_usdt))) {
      setAutoMaxQuote(String(p.max_quote_per_order_usdt));
    }
    if (p.limit != null && Number.isFinite(Number(p.limit))) {
      setAutoLimit(String(p.limit));
    }
    const sm = p.strategy_mode;
    if (sm === "daily_intraday" || sm === "trend_swing") setAutoStrategyMode(sm);
  }, []);
  ...
  const loadAutoCycles = useCallback(async (limit = 15) => {
    setAutoCyclesLoading(true);
    try {
      const res = await getCryptoTestnetAutoCycles(limit);
      if (!res.ok) {
        setAutoCyclesError(res.error ?? "No se pudo leer ciclos Auto Testnet");
        setAutoCycles([]);
        return;
      }
      setAutoCycles(res.cycles);
      setAutoCyclesError(null);
    } catch (e: unknown) {
      setAutoCyclesError(e instanceof Error ? e.message : "Error ciclos Auto Testnet");
      setAutoCycles([]);
    } finally {
      setAutoCyclesLoading(false);
    }
  }, []);

  const loadStrategyAnalysis = useCallback(async () => {
    setStrategyAnalysisLoading(true);
    setStrategyAnalysisError(null);
    try {
      const r = await getCryptoTestnetStrategyAnalysis();
      setStrategyAnalysis(r);
    } catch (e: unknown) {
      setStrategyAnalysisError(e instanceof Error ? e.message : "Error análisis estrategia");
      setStrategyAnalysis(null);
    } finally {
      setStrategyAnalysisLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (activeTestnetModule !== "auto_testnet") return undefined;
    void loadAutoStatus();
    void loadAutoCycles(15);
    void loadStrategyAnalysis();
    return undefined;
  }, [activeTestnetModule, loadAutoStatus, loadAutoCycles, loadStrategyAnalysis]);
  ...
  const canUseAutoTrading = useMemo(
    () =>
      Boolean(
        connected &&
        status?.sandbox_mode &&
        (status?.urls_api_safe === "sandbox" || status?.urls_api_safe === "testnet") &&
        status?.diagnosis === "ok",
      ),
    [connected, status?.sandbox_mode, status?.urls_api_safe, status?.diagnosis],
  );
  ...
  const navigateAfterOrderSuccess = useCallback((mod: TestnetModule, positionTab?: "open" | "closed") => {
    setPortfolioAssetsExpanded(false);
    setActiveTestnetModule(mod);
    if (positionTab && mod === "position") {
      setAppPositionsTab(positionTab);
    }
    window.requestAnimationFrame(() => {
      document.getElementById(`crypto-testnet-module-${mod}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  ...
  const executeTestnetMarketBuy = useCallback(
    async (opts: {
      ...
      if (isEntryProposalExecuted(sym)) {
        return {
          ok: false,
          error: `Ya existe una posición Testnet registrada por la app para ${sym} o la entrada ya fue ejecutada.${TESTNET_DUPLICATE_POSITION_NOTE}`,
        };
      }
      ...
      try {
        ...
        const mod = opts.successModule ?? activeTestnetModuleRef.current;
        navigateAfterOrderSuccess(mod, mod === "position" ? "open" : undefined);
        setError(null);
        return { ok: true };
      } catch (e: unknown) {
        return {
  ...
  const executeTestnetMarketSell = useCallback(
    async (opts: {
      ...
      try {
        ...
        const mod = opts.successModule ?? activeTestnetModuleRef.current;
        const tab = appPositionsTabAfterSell(appPos, sym);
        setAppPositionsTab(tab);
        navigateAfterOrderSuccess(mod, mod === "position" ? tab : undefined);
        setError(null);
        return { ok: true };
      } catch (e: unknown) {
        return {
  ...
  const submitTestnetManualOrder = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        ...
        setAppPositionFeedbackError(null);
          await refreshAfterTestnetOrder();
          navigateAfterOrderSuccess("operate");
          setError(null);
          return;
        }
        ...
        setAppPositionFeedbackError(null);
        const appPos = await refreshAfterTestnetOrder();
        const tab = appPositionsTabAfterSell(appPos, symTrim);
        setAppPositionsTab(tab);
        navigateAfterOrderSuccess("operate");
        setError(null);
      } catch (err: unknown) {
  ...
  const handleSellFromAppPosition = useCallback(
    async (opts: { symbol: string; asset: string; amountBase: number }) => {
      ...
      const result = await executeTestnetMarketSell({
        ...
        fromProposal: false,
        successModule: activeTestnetModuleRef.current,
      });
      if (result.ok) {
        setAppPositionFeedbackMessage(TESTNET_SELL_SUCCESS_MESSAGE);
        setOrderSuccessMessage(TESTNET_SELL_SUCCESS_MESSAGE);
  ...
  const autoPhaseLabel = useMemo(() => {
    if (!autoStatus?.enabled) return "Detenido";
    if (autoStatus.running) return "Ejecutando ciclo";
  ...
  const handleAutoStop = useCallback(async () => {
    ...
    setAutoCyclesError(null);
    try {
      const s = await postCryptoTestnetAutoStop();
      setAutoStatus(s);
      void loadAutoCycles(15);
    } catch (e: unknown) {
      setAutoCyclesError(e instanceof Error ? e.message : "No se pudo detener Auto Testnet");
    } finally {
      setAutoActionBusy(false);
    }
  }, [loadAutoCycles]);
  ...
  return (
    <div className="crypto-testnet-dashboard">
            ...
            {assistedPayload.proposal ? (
              <>
                ...
                <ul className="msg-muted" style={{ fontSize: "0.82rem", margin: "0.35rem 0 0", paddingLeft: "1.15rem" }}>
                  ...
                  <li>
                    Activar break-even al ganar {assistedPayload.proposal.risk.break_even_trigger_pct}% · Stop break-even{" "}
      ...
      <section className="card crypto-testnet-section">
          ...
          <div className="crypto-monitor-config-row crypto-monitor-config-row--risk">
            <div className="crypto-monitor-config-card crypto-monitor-config-card--risk">
              ...
              <div className="crypto-monitor-risk-grid">
                <label className="crypto-testnet-field crypto-monitor-field" title={MONITOR_SL_TOOLTIP}>
                ...
                <label className="crypto-testnet-field crypto-monitor-field" title={MONITOR_TP_TOOLTIP}>
                ...
                <label className="crypto-testnet-field crypto-monitor-field" title={MONITOR_TRAIL_TOOLTIP}>
              ...
              <div className="crypto-monitor-break-even-block">
                <h6 className="crypto-monitor-break-even-title">Break-even (opcional)</h6>
                <div className="crypto-monitor-break-even-grid">
                  <label className="crypto-testnet-field crypto-monitor-field" title={MONITOR_BE_ACTIVATE_TOOLTIP}>
                    <span className="crypto-testnet-field-label crypto-monitor-field-label">
                      Activar break-even al ganar %
                    </span>
                    <input
                      type="number"
                      className="radar-input crypto-testnet-input crypto-monitor-input"
                  ...
                  <label className="crypto-testnet-field crypto-monitor-field" title={MONITOR_BE_STOP_TOOLTIP}>
                ...
                <p className="msg-muted crypto-monitor-break-even-hint">
                  Equivale a un trigger de break-even (habilita la protección cuando la posición gana ese %). El stop
                  fija la salida cerca del precio de compra.
                </p>
              </div>
            </div>
          </div>
        ...
        <p className="msg-muted crypto-monitor-safe-hint crypto-monitor-future-auto-note">{MONITOR_FUTURE_AUTO_NOTE}</p>
      ...
      {activeTestnetModule === "auto_testnet" ? (
        <div className="crypto-testnet-module" id="crypto-testnet-module-auto_testnet" role="tabpanel">
          <TestnetModuleHeader
            title="Auto Testnet"
            lead="Runner en backend: órdenes MARKET sólo en Binance Spot Testnet/Sandbox. Guarda de sandbox antes de cada orden; registro en data/crypto_testnet_auto_cycles.jsonl."
          />
          <section className="card crypto-testnet-section">
            <h3 className="dashboard-section-title crypto-testnet-section-title">Auto Testnet (sandbox)</h3>
            <p className="msg-muted" style={{ margin: "0 0 0.75rem", fontSize: "0.88rem", lineHeight: 1.45 }}>
              No opera Binance real. Si el estado deja de confirmar testnet/sandbox, el ciclo aborta. Pérdida diaria
              estimada (sólo cierres auto) y tope de entradas detienen nuevas compras o apagan el runner según
              configuración.
            </p>
            {!canUseAutoTrading ? (
              <p className="crypto-testnet-note crypto-testnet-note--neutral" style={{ marginBottom: "0.65rem" }}>
                Conectá testnet en <strong>Estado</strong> (sandbox detectado, diagnosis <code>ok</code>) para activar el
                auto runner.
              </p>
            ) : null}

            <div className="crypto-testnet-mini-grid crypto-testnet-mini-grid--dense crypto-monitor-kpi-grid" style={{ marginBottom: "0.65rem" }}>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">Fase</span>
                <span className="crypto-testnet-kpi-value">{autoPhaseLabel}</span>
              </div>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">Runner</span>
                <span className="crypto-testnet-kpi-value">{autoStatus?.enabled ? "Activado" : "Detenido"}</span>
              </div>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">Última acción</span>
                <span className="crypto-testnet-kpi-value" style={{ fontSize: "0.82rem" }}>
                  {autoStatus?.last_action ?? "—"}
                </span>
              </div>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">PnL total (app)</span>
                <span className="crypto-testnet-kpi-value">
                  {autoStatus?.app_total_pnl_usdt != null && Number.isFinite(Number(autoStatus.app_total_pnl_usdt))
                    ? fmtNum(Number(autoStatus.app_total_pnl_usdt))
                    : "—"}
                </span>
              </div>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">PnL día (auto est.)</span>
                <span className="crypto-testnet-kpi-value">
                  {autoStatus?.auto_daily_pnl_usdt != null && Number.isFinite(Number(autoStatus.auto_daily_pnl_usdt))
                    ? fmtNum(Number(autoStatus.auto_daily_pnl_usdt))
                    : "—"}
                </span>
              </div>
              <div className="crypto-testnet-kpi">
                <span className="crypto-testnet-kpi-label">Entradas hoy / máx</span>
                <span className="crypto-testnet-kpi-value">
                  {`${autoStatus?.auto_entries_today ?? "—"} / ${(autoStatus?.params as { max_trades_per_day?: number } | undefined)?.max_trades_per_day ?? "—"}`}
                </span>
              </div>
            </div>
            {autoStatus?.stop_reason ? (
              <p className="msg-muted" style={{ margin: "0 0 0.5rem", fontSize: "0.82rem" }}>
                Motivo última detención: <strong>{autoStatus.stop_reason}</strong>
              </p>
            ) : null}
            {autoStatus?.last_error ? (
              <p className="msg-error" style={{ margin: "0 0 0.5rem", fontSize: "0.82rem" }}>
                {autoStatus.last_error}
              </p>
            ) : null}
            {autoCyclesError ? (
            ...
            {autoStatus?.last_guard_error_code ? (
              <div
                className="crypto-testnet-note crypto-testnet-note--neutral"
                style={{
                  margin: "0 0 0.65rem",
                ...
                role="status"
              >
                <strong>Último error de guardia</strong>
                <div style={{ marginTop: "0.25rem", fontFamily: "monospace", fontSize: "0.8rem" }}>
                  {autoStatus.last_guard_error_code}
                </div>
                {autoStatus.guard_first_failure_cycle_at ? (
                  <p className="msg-muted" style={{ margin: "0.35rem 0 0", fontSize: "0.8rem" }}>
                    Primer ciclo con fallo (UTC):{" "}
                    <strong>{fmtIsoLocalShort(String(autoStatus.guard_first_failure_cycle_at))}</strong>
                  </p>
                ) : null}
                {autoStatus.guard_advice ? (
                  <p style={{ margin: "0.35rem 0 0", fontSize: "0.82rem" }}>{autoStatus.guard_advice}</p>
                ) : (
                  <p style={{ margin: "0.35rem 0 0", fontSize: "0.82rem" }}>
                    Revisar credenciales/permisos Testnet o regenerar claves si expiraron.
                  </p>
                )}
                {autoStatus.guard_last_failure_snapshot &&
                typeof autoStatus.guard_last_failure_snapshot === "object" ? (
                  <p className="msg-muted" style={{ margin: "0.35rem 0 0", fontSize: "0.76rem" }}>
                    Diagnosis:{" "}
                    <code>{String((autoStatus.guard_last_failure_snapshot as Record<string, unknown>).diagnosis ?? "—")}</code>
                    {" · "}
                    balance:{" "}
                    <code>
                      {String(
                        (autoStatus.guard_last_failure_snapshot as Record<string, unknown>).can_read_balance ?? "—",
                      )}
                    </code>
                    {" · "}
                    ticker:{" "}
                    <code>
                      {String(
                        (autoStatus.guard_last_failure_snapshot as Record<string, unknown>).can_read_ticker ?? "—",
                      )}
                    </code>
                  </p>
                ) : null}
            ...
            <div className="crypto-testnet-toolbar crypto-testnet-block-start" style={{ gap: "0.5rem", flexWrap: "wrap" }}>
              <button
                type="button"
              ...
              <button
                type="button"
                className="radar-refresh-btn crypto-testnet-btn-auto-stop"
                onClick={() => void handleAutoStop()}
                disabled={autoActionBusy || !autoStatus?.enabled}
              >
                {autoActionBusy && autoStatus?.enabled ? "Deteniendo…" : "Detener Auto Testnet"}
              </button>
              <button
                type="button"
                className="radar-refresh-btn crypto-testnet-btn-compact"
            ...
            <div className="crypto-monitor-cycles-block crypto-testnet-block-start" style={{ marginTop: "0.85rem" }}>
              <div className="crypto-testnet-section-head">
                <div>
                  ...
                  <p className="msg-muted" style={{ margin: "0.3rem 0 0", fontSize: "0.78rem" }}>
                    Misma lógica que <code>propose_testnet_exits</code> (monitor / auto); sólo diagnóstico, sin órdenes.
                    {autoStatus?.open_position_risk?.evaluated_at ? (
                      <>
                        {" "}
                        Evaluado: {fmtIsoLocalShort(String(autoStatus.open_position_risk.evaluated_at))}
                      </>
                    ) : null}
                  </p>
                </div>
              </div>
              {!autoStatus?.open_position_risk?.ok ? (
                <p className="msg-error" style={{ margin: "0.5rem 0 0", fontSize: "0.82rem" }}>
                  {autoStatus?.open_position_risk?.error ?? "Sin datos de riesgo"}
                </p>
              ) : (autoStatus.open_position_risk.positions?.length ?? 0) === 0 ? (
                <p className="msg-muted" style={{ margin: "0.5rem 0 0", fontSize: "0.82rem" }}>
            ...
            <div className="crypto-monitor-cycles-block crypto-testnet-block-start" style={{ marginTop: "0.85rem" }}>
                ...
                <div className="crypto-testnet-mini-grid crypto-testnet-mini-grid--dense" style={{ marginTop: "0.25rem" }}>
                  <div className="crypto-testnet-kpi">
                    ...
                    <span className="crypto-testnet-kpi-value" style={{ fontSize: "0.78rem" }}>
                      {fmtIsoLocalShort(
                        String(
                          (autoStatus.last_cycle_record as Record<string, unknown>).cycle_started_at ??
                            (autoStatus.last_cycle_record as Record<string, unknown>).timestamp ??
                            "",
                        ),
                      )}
                    </span>
                  ...
                  <div className="crypto-testnet-kpi">
                    ...
                    <span className="crypto-testnet-kpi-value" style={{ fontSize: "0.78rem" }}>
                      {fmtIsoLocalShort(
                        String((autoStatus.last_cycle_record as Record<string, unknown>).cycle_finished_at ?? ""),
                      )}
                    </span>
                  </div>
                  <div className="crypto-testnet-kpi">
                    ...
                    <span className="crypto-testnet-kpi-value">
                      {(autoStatus.last_cycle_record as Record<string, unknown>).duration_ms != null
                        ? `${String((autoStatus.last_cycle_record as Record<string, unknown>).duration_ms)} ms`
                        : "—"}
                    </span>
                  </div>
                  <div className="crypto-testnet-kpi">
                    <span className="crypto-testnet-kpi-label">Estado ciclo</span>
                    <span className="crypto-testnet-kpi-value" style={{ fontSize: "0.78rem" }}>
                      {String((autoStatus.last_cycle_record as Record<string, unknown>).status ?? "—")}
                    </span>
                  </div>
                </div>
              ) : (
              ...
              {autoStatus?.last_cycle_record &&
              Array.isArray((autoStatus.last_cycle_record as Record<string, unknown>).errors) &&
              ((autoStatus.last_cycle_record as Record<string, unknown>).errors as unknown[]).length > 0 ? (
                <p className="msg-error" style={{ margin: "0.45rem 0 0", fontSize: "0.78rem" }}>
            ...
            <div className="crypto-monitor-cycles-block crypto-testnet-block-start" style={{ marginTop: "0.85rem" }}>
              <div className="crypto-testnet-section-head">
                <div>
                  <h4 className="crypto-monitor-subsection-title" style={{ margin: 0 }}>
                    Análisis de estrategia Testnet
                  </h4>
                  <p className="msg-muted" style={{ margin: "0.3rem 0 0", fontSize: "0.78rem" }}>
                    Diagnóstico basado en operaciones Testnet. No modifica parámetros automáticamente.
                  </p>
                </div>
                <div className="crypto-testnet-toolbar">
                  <button
                    type="button"
                    className="radar-refresh-btn crypto-testnet-btn-compact"
                    onClick={() => void loadStrategyAnalysis()}
                    disabled={strategyAnalysisLoading}
                  >
                    {strategyAnalysisLoading ? "…" : "Refrescar análisis"}
                  </button>
                </div>
              </div>
              ...
              {strategyAnalysis?.ok ? (
                <div className="crypto-testnet-mini-grid crypto-testnet-mini-grid--dense" style={{ marginTop: "0.45rem" }}>
                  <div className="crypto-testnet-kpi">
                    <span className="crypto-testnet-kpi-label">Trades cerrados</span>
                    <span className="crypto-testnet-kpi-value">{strategyAnalysis.closed_trades_count ?? "—"}</span>
                  </div>
                  <div className="crypto-testnet-kpi">
                    <span className="crypto-testnet-kpi-label">Win rate</span>
                    <span className="crypto-testnet-kpi-value">
                      {strategyAnalysis.win_rate_pct != null && Number.isFinite(Number(strategyAnalysis.win_rate_pct))
                        ? `${fmtNum(Number(strategyAnalysis.win_rate_pct))}%`
                        : "—"}
                    </span>
                  </div>
                  <div className="crypto-testnet-kpi">
                    ...
                    <span className="crypto-testnet-kpi-value">
                      {strategyAnalysis.total_pnl_usdt != null && Number.isFinite(Number(strategyAnalysis.total_pnl_usdt))
                        ? fmtNum(Number(strategyAnalysis.total_pnl_usdt))
                        : "—"}
                    </span>
                  </div>
                  <div className="crypto-testnet-kpi">
                    <span className="crypto-testnet-kpi-label">Fuente cierres</span>
                    <span className="crypto-testnet-kpi-value" style={{ fontSize: "0.72rem" }}>
                      {strategyAnalysis.closed_trades_source ?? "—"}
                    </span>
                  </div>
                </div>
              ) : strategyAnalysisLoading ? (
                <p className="msg-muted" style={{ margin: "0.45rem 0 0", fontSize: "0.82rem" }}>
                  Cargando análisis…
                </p>
              ) : (
                <p className="msg-muted" style={{ margin: "0.45rem 0 0", fontSize: "0.82rem" }}>
                  Sin datos de análisis todavía.
                </p>
              )}
              {strategyAnalysis?.ok && strategyAnalysis.by_setup && Object.keys(strategyAnalysis.by_setup).length > 0 ? (
                <div style={{ marginTop: "0.55rem" }}>
                  <h5 className="crypto-monitor-config-card-heading" style={{ margin: "0 0 0.35rem" }}>
                    Por setup (órdenes / FIFO)
                  </h5>
                  <div className="table-wrap crypto-testnet-cycle-list-compact">
                    <table className="crypto-testnet-table crypto-testnet-table--compact">
                      <thead>
                        <tr>
                          <th>Setup</th>
                          <th className="crypto-testnet-num">Trades</th>
                          <th className="crypto-testnet-num">Win %</th>
                          <th className="crypto-testnet-num">PnL USDT</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(strategyAnalysis.by_setup)
                          .sort((a, b) => (b[1].pnl_usdt ?? 0) - (a[1].pnl_usdt ?? 0))
                          .map(([setup, agg]) => (
                            <tr key={setup}>
                              <td>{setup}</td>
                              <td className="crypto-testnet-num">{agg.trades ?? "—"}</td>
                              <td className="crypto-testnet-num">
                                {agg.win_rate_pct != null && Number.isFinite(Number(agg.win_rate_pct))
                                  ? `${fmtNum(Number(agg.win_rate_pct))}%`
                                  : "—"}
                              </td>
                              <td className="crypto-testnet-num">
                                {agg.pnl_usdt != null && Number.isFinite(Number(agg.pnl_usdt))
                                  ? fmtNum(Number(agg.pnl_usdt))
                                  : "—"}
                              </td>
                            </tr>
                          ))}
              ...
              {strategyAnalysis?.ok && Array.isArray(strategyAnalysis.recommendations) && strategyAnalysis.recommendations.length > 0 ? (
                <div style={{ marginTop: "0.55rem" }}>
                  <h5 className="crypto-monitor-config-card-heading" style={{ margin: "0 0 0.35rem" }}>
                    Recomendaciones (orientativas)
                  </h5>
                  <ul style={{ margin: 0, paddingLeft: "1.1rem", fontSize: "0.82rem", lineHeight: 1.45 }}>
                    {strategyAnalysis.recommendations.map((rec, i) => (
                      <li key={`${rec.code ?? "r"}-${i}`}>{rec.text ?? rec.code}</li>
                    ))}
                  </ul>
                </div>
              ) : null}
            ...
            <div className="crypto-monitor-cycles-block">
              <div className="crypto-testnet-section-head">
                <div>
                  <h4 className="crypto-monitor-subsection-title" style={{ margin: 0 }}>
                    Ciclos recientes (JSONL)
                  </h4>
                  <p className="msg-muted" style={{ margin: "0.3rem 0 0", fontSize: "0.78rem" }}>
                    Auditoría local: <code>data/crypto_testnet_auto_cycles.jsonl</code>
                  </p>
                </div>
                <div className="crypto-testnet-toolbar">
                  <button
                    type="button"
                    className="radar-refresh-btn crypto-testnet-btn-compact"
                    onClick={() => void loadAutoCycles(30)}
                    disabled={autoCyclesLoading}
                  >
                    {autoCyclesLoading ? "…" : "Refrescar"}
                  </button>
                </div>
              </div>
              {autoCycles.length > 0 ? (
                    ...
                    <tbody>
                      {autoCycles.slice(0, 12).map((row, idx) => (
                        <tr key={`${String(row.timestamp ?? row.cycle_finished_at ?? "t")}-${idx}`}>
                          ...
                          <td style={{ fontSize: "0.76rem", maxWidth: "18rem", wordBreak: "break-word" }}>
                            {JSON.stringify(row.actions_taken ?? [])}
                          </td>
                          <td style={{ fontSize: "0.76rem", maxWidth: "12rem", wordBreak: "break-word" }}>
